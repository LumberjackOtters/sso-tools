<template>
  <div>
    ...
    <v-alert class="mt-3 mb-3">
      <h3>TL;DR?</h3>
      <p>If you're already familiar with OAuth2 and OpenID Connect, then <router-link :to="`/idps/${idp._id}/oauth`">check out the available URLs and scopes</router-link> to get started.</p>
    </v-alert>
    ...
    <p class="mt-2">Under this approach, the app can avoid having to do additional lookups (using the access token) to the IdP in order to retrieve the required user profile information to log the user in.</p>
    <p class="mt-2">SSO Tools supports OAuth2 with or without OpenID Connect.</p>
    ...
    <p class="mt-2">When creating your app, you'll need to provide a "redirect URI". This the place the user will be redirected back to after successful authentication. If you don't know this right away, you can always change it later.</p>
    ...
    <p class="mt-2">Along with this request, you'll have to include some additional query parameters:</p>
    <ul class="mt-2">
      <li>`client_id`: The client ID for the app generated by your IdP (this can be found on the IdP's connected apps page).</li>
      <li>`scope`: The comma-separated list of requested permissions.</li>
    ...
    <p class="mt-2">For the scope parameter, SSO Tools currently supports "email" (to allow reading the user's email address), "profile" (to allow reading the user's name and profile information), and "openid" (to receive an ID token after authentication).</p>
    <p class="mt-2">As such, an example request to redirect the user to on SSO Tools could look like the following:</p>
    <p class="mt-2">https://idp.sso.tools/myidp/oauth2/authorize?client_id=abc123&scope=profile,email,openid&redirect_uri=https://myapp.com/oauth/callback&response_type=code</p>
    ...
    <p class="mt-2">When the user arrives back, they'll have a `code` parameter in their request URL. For example, if your redirect URI is "https://myapp.com/oauth/callback" your app will receive a request at "https://myapp.com/oauth/callback?code=xzy321"</p>
    <p class="mt-2">Your app should read this code and make it available for the next step</p>
    ...
    <p class="mt-2">For example, given the information above, a sample token request could look like this:</p>
    <pre class="mt-2">
...

{
  ...
  "grant_type": "authorization_code"
    ...
    <p class="mt-2">The "access_token" received in Step 5 can now be used against the IdP's API in order to retrieve the user's information. To do so, include the access token in the Authorization header and send off a request to "https://idp.sso.tools/ISSUER/api/users/me".</p>
    <p class="mt-2">For example, given the above information, your request could look like this:</p>
    <pre class="mt-2">
GET https://idp.sso.tools/myidp/api/users/me
Accept: application/json
Authorization: ACCESSTOKEN
    </pre>
    <p class="mt-2">Assuming all went well, the IdP will respond with information about the current user, as defined in the requested scopes.</p>
