<template>
  <div>
    ...
    <p>In your app, create a button or link that redirects the user to the IdP's "authorization URL". In SSO Tools, these take the form of "${ idpUrl }/ISSUER/oauth2/authorize", replacing "ISSUER" with the issuer code for your IdP (e.g. "myidp").</p>
    ...
    <p class="mt-4">${ idpUrl }/myidp/oauth2/authorize?client_id=abc123&scope=profile%20email%20openid&redirect_uri=https://myapp.com/oauth/callback&response_type=code</p>
    ...
    <p>Next, your app will need to make a request directly to the IdP on its "token" endpoint. On SSO Tools, these endpoints look like "${ idpUrl }/ISSUER/oauth2/token". To this endpoint, your app should send an HTTP `POST` request containing the following data in JSON format:</p>
    <ul class="mt-4">
    ...
    <pre class="mt-4">
POST ${ idpUrl }/myidp/oauth2/token
...

{
  ...
  "grant_type": "authorization_code"
    ...
    <p class="mt-4">The "access_token" received in Step 5 can now be used against the IdP's API in order to retrieve the user's information. To do so, include the access token in the Authorization header and send off a request to "${ idpUrl }/ISSUER/api/users/me".</p>
    <p class="mt-4">For example, given the above information, your request could look like this:</p>
    <pre class="mt-4">
GET ${ idpUrl }/myidp/api/users/me
Accept: application/json
